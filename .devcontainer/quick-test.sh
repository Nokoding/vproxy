...
for site in $SITES; do
  check "http://127.0.0.1:$LOCAL_HTTP_PORT" "" "$site" "plain" || FAIL=1
done

if [ -f /tmp/vproxy-tls.pid ] && kill -0 "$(cat /tmp/vproxy-tls.pid)" 2>/dev/null; then
  for site in $SITES; do
    # --proxy-insecure: the cert is issued for the DuckDNS domain, not
    ...
    check "https://127.0.0.1:$LOCAL_TLS_PORT" "--proxy-insecure" "$site" "tls  " || FAIL=1
  done
else
